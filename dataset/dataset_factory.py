...
def get_scanrefer_task_dataset(split="train", tokenizer=None, txt_seq_length=50, pc_seq_length=80, **args):
    ...
    return ScanFamilyDatasetWrapper(
        dataset=dataset, tokenizer=tokenizer, max_seq_length=txt_seq_length, max_obj_len=pc_seq_length
    )


@registry.register_dataset("scannetpp_task")
def get_scannetpp_task_dataset(tokenizer=None, txt_seq_length=50, pc_seq_length=80, **args):
    tokenizer = registry.get_language_model(tokenizer)()
    dataset = OVFGVGDataset(max_obj_len=pc_seq_length, **args)
    return ScanFamilyDatasetWrapper(
        dataset=dataset, tokenizer=tokenizer, max_seq_length=txt_seq_length, max_obj_len=pc_seq_length
    )
...
def get_caption_task_dataset(
    ...
    **args
    ...
    return CaptionDatasetWrapper(
        dataset, tokenizer, vocab, corpus, txt_seq_length, pc_seq_length, txt_mask_ratio, split
    )
